...
import { getConnectedRepositories,disconnectRepository,disconnectAllRepository } from "../actions";
...
export function RepositoryList(){
    ...
    const disconnectMutation=useMutation({
        ...
        onSuccess:(result)=>{
            if(result?.success){
                queryClinet.invalidateQueries({queryKey:["connected-repositories"]})
                queryClinet.invalidateQueries({queryKey:["dashboard-stats"]})
            ...
            <CardContent>
                {!repositories || repositories.length===0?(
                    <div className="text-center py-8 text-muted-foreground">
                        ...
                        <p className="text-sm-2">Connect repositories from the Repository page.</p>
                    </div>
                ):(
                    <div className="space-y-4">
                        {repositories.map((repo)=>(
                            <div key={repo.id}
                            className="flex items-center justify-between p-4 border rounded-lg hover:bg-mutation/50 transition-colors"
                            >
